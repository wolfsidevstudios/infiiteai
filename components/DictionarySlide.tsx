...
const DictionarySlide: React.FC<Props> = ({ terms }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col md:flex-row bg-white overflow-hidden rounded-3xl border border-gray-100 shadow-sm">
      {/* Sidebar List */}
      <div className="w-full md:w-1/3 bg-gray-50 border-r border-gray-200 flex flex-col">
         <div className="p-4 border-b border-gray-200">
             <form onSubmit={handleSearch} className="relative">
                 <input 
                    ...
                    className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-black/5 transition-all bg-white"
                 />
         ...
         <div className="flex-1 overflow-y-auto no-scrollbar p-2">
            <h4 className="px-4 py-2 text-xs font-bold text-gray-400 uppercase tracking-widest">Key Terms</h4>
            {filteredTerms.map(term => (
                <button
                    ...
                    className={`
                        ...
                        ${selectedWord === term ? 'bg-black text-white shadow-lg' : 'hover:bg-gray-200 text-gray-700'}
                    `}
                >
                    <span className="font-medium capitalize">{term}</span>
                    {selectedWord === term && <BookA size={16} className="text-white/50" />}
                </button>
            ))}
            {filteredTerms.length === 0 && searchTerm && (
                <button
                    onClick={() => setSelectedWord(searchTerm)}
                    className="w-full text-left px-4 py-3 rounded-xl mb-1 hover:bg-gray-200 text-blue-600 font-medium"
                >
                    Look up "{searchTerm}"
                </button>
            )}
          ...
          {loading ? (
             ...
             <div className="animate-fade-in max-w-2xl mx-auto w-full">
                 <div className="flex items-start justify-between mb-8">
                     <div>
                         <h2 className="text-5xl md:text-6xl font-bold text-gray-900 mb-2 capitalize tracking-tight">{definition.word}</h2>
                         <p className="text-2xl text-gray-500 font-serif italic">{definition.phonetic}</p>
                     </div>
                     {definition.audio && (
                         <button 
                            onClick={playAudio}
                            className="p-4 bg-black text-white rounded-full hover:scale-110 transition-transform shadow-lg"
                         >
                             <Volume2 size={24} />
                         </button>
                     )}
                     ...
                     {definition.meanings.map((meaning, idx) => (
                         <div key={idx} className="border-b border-gray-100 pb-6 last:border-0">
                             <div className="flex items-center gap-3 mb-4">
                                 <span className="px-3 py-1 bg-gray-100 text-gray-600 rounded-lg text-sm font-bold font-mono uppercase">
                             ...
                             <ul className="space-y-4">
                                 {meaning.definitions.slice(0, 3).map((def, dIdx) => (
                                     <li key={dIdx} className="text-lg text-gray-800 leading-relaxed pl-4 border-l-2 border-black/10">
                                         {def.definition}
                                         {def.example && (
                                             <p className="mt-2 text-gray-500 italic text-base">"{def.example}"</p>
                                         )}
                                     </li>
                                 ))}
                             </ul>
